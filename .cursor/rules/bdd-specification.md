...
- 位置：与被测文件同级目录
- 示例：`device-service.ts` → `device-service.test.ts`
...
// 自定义数据
const customDevice = deviceFactory({ name: 'Custom Device', status: 'offline' });
...
1. **AAA模式**：Arrange (Given) → Act (When) → Assert (Then)
